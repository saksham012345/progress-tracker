...
const TopicDetail = () => {
    ...
    const fetchData = async () => {
        try {
            const topicRes = await fetch(`http://localhost:5000/api/topics`); // Filter locally for now or better endpoint
            ...
            const sessionsRes = await fetch(`http://localhost:5000/api/sessions/${id}`);
    ...
    const handleStatusChange = async (newStatus) => {
        try {
            const res = await fetch(`http://localhost:5000/api/topics/${id}/status`, {
                method: 'PATCH',
    ...
    const handleImproveNotes = async (notes) => {
        ...
        const res = await fetch('http://localhost:5000/api/ai/improve-notes', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
